...
namespace Projects_Management_System_Naseej.Controllers
{
            ...
            var contentTypes = new Dictionary<string, string>
    {
        ...
        [HttpGet("view-in-drive/{googleDriveFileId}")]
        public async Task<IActionResult> ViewInDrive(string googleDriveFileId)
        {
            try
            {
                string webViewLink = await _googleDriveService.GetFileWebViewLink(googleDriveFileId);
                return Redirect(webViewLink);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error getting Google Drive web view link");
                return StatusCode(500, "An error occurred while retrieving file view link");
            }
        }
